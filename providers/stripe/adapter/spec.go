...
func describeActionCatalog() []contract.IntegrationActionDefinition {
	catalog := []contract.IntegrationActionDefinition{
		...
		{Name: OperationOnSurfaceQuery, Description: "Surface-driven read aggregator invoked by core's /api/v1/integrations/{instance_id}/surface-query proxy. Accepts { query_name, params }; routes by query_name to a read aggregator. Supported: list-webhook-endpoints (webhook-health pillar — {id,url,status,enabled_events,api_version}), get-balance (available + pending arrays per currency, smallest unit), list-charges (recent charges for reconciliation — {id,amount,currency,status,created,refunded,payment_intent}), list-subscriptions ({id,status,plan{nickname,price_id},amount,currency,current_period_end,cancel_at_period_end}; customer as opaque id ref only), list-payment-intents ({id,status,amount,currency,created,capture_method}), charge-detail (drill-down by charge_id — {id,amount,currency,status,created,refunded,refundedAmount,payment_intent,failureCode,failureMessage,refunds[]}). Customer PII intentionally not projected — opaque refs only. Read-only; never mutates.", ResourceTypes: []string{resourceWebhookEndpoint}, Idempotent: true},
