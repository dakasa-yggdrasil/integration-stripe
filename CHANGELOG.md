...
- Preserve all four by-ID observation results through the public RPC reconcile
  bridge by consistently returning an items array. Disabled webhooks remain
  observable using one provider GET and never expose signing secrets.
- Reject malformed observation envelopes instead of returning successful null
  items. Preserve non-null empty arrays for authoritative absence.
- Treat provider HTTP 404 as absence for webhook observation and as idempotent
  success for webhook deletion. Permission and transport errors still fail.
